...
### 你需要准备的东西：
- [ ] **火山云服务器公网 IP**（如：123.456.789.0）
- [ ] **SSH 密钥文件**（.pem 格式，如：volcano-key.pem）
- [ ] **域名**（如：example.com，已解析到服务器 IP）
- [ ] **火山云 PostgreSQL 连接信息**：
  - 内网地址（如：10.0.0.1）
  - 端口（通常是 5432）
  - 数据库名（如：postgres）
...
```bash
# 将密钥文件权限设置为只有所有者可读
chmod 600 volcano-key.pem

# 如果你的密钥文件名不是 volcano-key.pem，请替换为你的实际文件名
...
```bash
# 使用 SSH 密钥连接
ssh -i volcano-key.pem root@你的服务器公网IP

# 示例：
# ssh -i volcano-key.pem root@123.456.789.0
```
...
```bash
# 使用 SCP 上传整个项目
scp -i volcano-key.pem -r . root@你的服务器公网IP:/var/www/luowato-selection

# 示例：
# scp -i volcano-key.pem -r . root@123.456.789.0:/var/www/luowato-selection
```
...
# 应该显示：
# -rwxr-xr-x 1 root root 5.2K Jan 1 00:00 deploy-volcano.sh
```
...
修改为你的火山云 PostgreSQL 连接信息：
```bash
# 格式：postgresql://用户名:密码@内网地址:端口/数据库名
DATABASE_URL=postgresql://postgres:your-password@10.0.0.1:5432/postgres

# 示例：
# DATABASE_URL=postgresql://admin:MyPassword123@10.0.1.100:5432/luowato_selection
```
...
2. 点击"配置"按钮
3. 添加你的域名：
   - 域名：`example.com`（不要加 http:// 或 https://）
4. 点击"确定"
...
```bash
# 上传校验文件到服务器
scp -i volcano-key.pem MP_verify_xxxxxx.txt root@你的服务器公网IP:/var/www/luowato-selection/public/

# 示例：
# scp -i volcano-key.pem MP_verify_ABC123.txt root@123.456.789.0:/var/www/luowato-selection/public/
```
...
DNS 生效通常需要 **5-10 分钟**，最多可能需要 24 小时。
...
# 使用详细模式连接
ssh -v -i volcano-key.pem root@你的服务器公网IP
```
...
# 添加以下行（每天凌晨 2 点备份）
# 0 2 * * * pg_dump -U 用户名 数据库名 > /backup/db_$(date +\%Y\%m\%d).sql
```
